...
function navigate(page) {
  ...
  if (page !== 'dashboard' && typeof hideSectionJumpNav === 'function') hideSectionJumpNav();
...
function openProject(id) {
  ...
  if (!p) return;
  if (typeof hideSectionJumpNav === 'function') hideSectionJumpNav();
...
function renderDashboard(c) {
  ...
  // Build section jump-nav for mobile (only on long scrollable dashboards)
  buildSectionJumpNav(dashboardMode);
}
...
function renderProjectPage(c) {
  ...
  c.innerHTML = `
    <button class="floating-back-btn" onclick="closeProjectPage()" title="Back" aria-label="Back">
      <svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M9 2L4 7l5 5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
    </button>
...
// ── Section Jump Nav (mobile) ──
// Builds a horizontally scrollable chip nav above the bottom nav.
// Each chip jumps to a top-level dashboard-card section. Active chip tracks
// which section is currently in view via IntersectionObserver.

function buildSectionJumpNav(dashboardMode) {
  // Skip on non-long-scrolling dashboards
  const longModes = ['executive', 'install_mgmt', 'design_mgmt', 'sales_mgmt'];
  if (!longModes.includes(dashboardMode)) {
    hideSectionJumpNav();
    return;
  }
  ...
  // Find all top-level dashboard-cards with a recognizable title
  const cards = Array.from(content.querySelectorAll(':scope > .dashboard-card, :scope > div > .dashboard-card'));
  // Also pick up the metrics grid as the first jump target
  const metricsGrid = content.querySelector('.metrics-grid, [class*="metric-card"]')?.closest('div[style*="grid"]');

  const sections = [];
  if (metricsGrid && !metricsGrid.dataset.jumpId) {
    metricsGrid.dataset.jumpId = 'metrics';
    sections.push({ id: 'metrics', label: 'Top' });
  }

  cards.forEach((card, i) => {
    const titleEl = card.querySelector('.dashboard-card-title');
    if (!titleEl) return;
    // Get short label from title (first word group, max ~14 chars)
    let label = titleEl.textContent.trim().replace(/\s+\u00B7\s+\d+.*$/, '').replace(/\s+&middot;.*$/, '');
    label = label.split('·')[0].trim();
    if (label.length > 18) label = label.slice(0, 16) + '…';
    if (!label) return;
    const id = 'sec-' + i;
    card.dataset.jumpId = id;
  ...
  // Get or create the jump nav element
  let nav = document.getElementById('section-jump-nav');
  if (!nav) {
    nav = document.createElement('div');
    nav.id = 'section-jump-nav';
    document.body.appendChild(nav);
  }
  nav.classList.add('active');
  document.body.classList.add('has-jump-nav');
  nav.innerHTML = '<div class="sjn-inner">' + sections.map((s, idx) => (
    '<div class="sjn-chip' + (idx === 0 ? ' active' : '') + '" data-target="' + s.id + '" onclick="jumpToSection(\'' + s.id + '\')">' + esc(s.label) + '</div>'
  )).join('') + '</div>';

  // Set up intersection observer for active state
  if (window._jumpObserver) window._jumpObserver.disconnect();
  window._jumpObserver = new IntersectionObserver((entries) => {
    // Find the entry most in view at the top of the scroll area
    const visible = entries.filter(e => e.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio);
    if (visible.length === 0) return;
    const topVisible = visible[0].target;
    const id = topVisible.dataset.jumpId;
    if (!id) return;
    nav.querySelectorAll('.sjn-chip').forEach(chip => {
      if (chip.dataset.target === id) chip.classList.add('active');
  ...
  sections.forEach(s => {
    const el = content.querySelector('[data-jump-id="' + s.id + '"]');
    if (el) window._jumpObserver.observe(el);
  });
}

function hideSectionJumpNav() {
  const nav = document.getElementById('section-jump-nav');
  if (nav) {
    nav.classList.remove('active');
    nav.innerHTML = '';
  }
  document.body.classList.remove('has-jump-nav');
  if (window._jumpObserver) {
    window._jumpObserver.disconnect();
    window._jumpObserver = null;
  }
}

function jumpToSection(id) {
  const target = document.querySelector('[data-jump-id="' + id + '"]');
  if (!target) return;
  // Scroll account: bottom-nav (~56px) + jump-nav (~38px) + a bit of breathing room
  const offset = 70;
...
// Hide jump nav when navigating away from dashboard
(function attachJumpNavCleanup() {
  if (window._jumpNavCleanupAttached) return;
  window._jumpNavCleanupAttached = true;
  // Re-evaluate on resize (so it disappears when rotating to landscape on tablet)
  window.addEventListener('resize', () => {
    const mode = state.dashboardMode;
    const onDashboard = state.currentPage === 'dashboard';
    if (onDashboard) {
      buildSectionJumpNav(mode);
    } else {
      hideSectionJumpNav();
    }
  });
