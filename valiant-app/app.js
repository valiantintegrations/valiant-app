  ...
  return `
    ...
    <div class="pipeline-grid">
      ${STAGES.map(s => {
        ...
        return `
        <div class="pipeline-col${col.length === 0 ? ' pipeline-col-empty' : ''}" ondragover="onDragOver(event)" ondragleave="onDragLeave(event)" ondrop="onDropStage(event, '${s.key}')">
...
function renderProjectPage(c) {
  ...
  const railHTML = `<button class="prail-mobile-back" onclick="closeProjectPage()" aria-label="Back" title="Back"><svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M9 2L4 7l5 5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></button>` + railItems.map(item => `
