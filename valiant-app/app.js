...
function renderProjectPage(c) {
  ...
  c.innerHTML = `
    <div class="project-mobile-topstrip">
      <button class="ptop-back" onclick="closeProjectPage()" aria-label="Back" title="Back">
      ...
      <div class="ptop-title">${esc(p.name)}</div>
    </div>
...
function updateContextNav() {
  ...
  if (page === 'project' && state.currentProject) {
    ...
    chips = railItems.map(it => ({
      ...
      action: `switchProjectTab('${it.key}');scrollContextNavTo('${it.key}')`
    }));
  ...
  inner.innerHTML = chips.map(c => (
    `<button class="ctx-chip${c.active ? ' active' : ''}" data-key="${c.key}" onclick="${c.action}">${esc(c.label)}</button>`
  )).join('');
...
function getDashboardContextChips() {
  ...
  const handler = (sectionKey) => `scrollToContextSection('${sectionKey}')`;

  if (mode === 'sales_mgmt') {
    return [
      { key: 'metrics',   label: 'Top',       action: handler('metrics') },
      { key: 'lead',      label: 'Lead',      action: handler('lead') },
      { key: 'proposal',  label: 'Proposal',  action: handler('proposal') },
      { key: 'sent',      label: 'Sent',      action: handler('sent') },
      { key: 'contract',  label: 'Contract',  action: handler('contract') },
      { key: 'stalled',   label: 'Stalled',   action: handler('stalled') },
      { key: 'by-lead',   label: 'By Lead',   action: handler('by-lead') }
    ];
  }
  if (mode === 'design_mgmt') {
    return [
      { key: 'metrics',     label: 'Top',        action: handler('metrics') },
      { key: 'kickoff',     label: 'Kickoff',    action: handler('kickoff') },
      { key: 'handoff',     label: 'Handoff',    action: handler('handoff') },
      { key: 'queue',       label: 'Queue',      action: handler('queue') },
      { key: 'templates',   label: 'Templates',  action: handler('templates') }
    ];
  }
  if (mode === 'install_mgmt') {
    return [
      { key: 'metrics',     label: 'Top',         action: handler('metrics') },
      { key: 'closeout',    label: 'Closeout',    action: handler('closeout') },
      { key: 'in-install',  label: 'In Install',  action: handler('in-install') },
      { key: 'upcoming',    label: 'Upcoming',    action: handler('upcoming') },
      { key: 'shop',        label: 'Shop Work',   action: handler('shop') },
      { key: 'stuck',       label: 'Stuck',       action: handler('stuck') }
    ];
  }
  if (mode === 'executive') {
    return [
      { key: 'metrics',     label: 'Metrics',    action: handler('metrics') },
      { key: 'pipeline',    label: 'Pipeline',   action: handler('pipeline') },
      { key: 'attention',   label: 'Attention',  action: handler('attention') },
      { key: 'activity',    label: 'Activity',   action: handler('activity') }
    ];
  }
  if (mode === 'pipeline') {
    return [
      ...
      { key: 'sent',      label: 'Sent',      action: handler('sent') },
      { key: 'contract',  label: 'Contract',  action: handler('contract') },
      { key: 'design',    label: 'Design',    action: handler('design') },
      { key: 'install',   label: 'Install',   action: handler('install') },
      { key: 'complete',  label: 'Complete',  action: handler('complete') }
    ];
  }
  ...
  if (mode === 'mine') {
    ...
    const chips = [];
    if (closeouts.length > 0) chips.push({ key: 'closeout', label: 'Closeout', action: handler('closeout') });
    if (myAssignments.sales?.length)     chips.push({ key: 'sales',     label: 'Sales',     action: handler('sales') });
    if (myAssignments.design?.length)    chips.push({ key: 'design',    label: 'Design',    action: handler('design') });
    if (myAssignments.pm?.length)        chips.push({ key: 'pm',        label: 'PM',        action: handler('pm') });
    if (myAssignments.install?.length)   chips.push({ key: 'install',   label: 'Install',   action: handler('install') });
    if (myAssignments.warehouse?.length) chips.push({ key: 'warehouse', label: 'Warehouse', action: handler('warehouse') });
    return chips;
  }
  return [];
}
