...
return `<div class="section-header"> <div class="section-title">Recently Updated</div> </div> <div class="card"> ${recent.map(p =>`
...
c.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;gap:12px;flex-wrap:wrap"> <input type="text" class="form-input" placeholder="Search projects…" id="proj-search" oninput="filterProjects()" style="max-width:300px"> <div style="display:flex;gap:6px;flex-wrap:wrap"> <button class="btn btn-sm ${!window._projFilter ? 'btn-primary' : ''}" onclick="setProjectFilter(null)" style="${!window._projFilter ? 'background:#1565C0;border-color:#1565C0;color:#fff' : ''}">All (${projects.length})</button> ${STAGES.map(s => { const cnt = projects.filter(p => p.stage === s.key).length; return`<button class="btn btn-sm" onclick="setProjectFilter('${s.key}')">${s.label} (${cnt})</button>`; }).join('')} </div> </div> <div class="card" style="padding:0;overflow-x:auto"> <table class="projects-table"> <thead> <tr> <th>Project</th> <th>Client</th> <th>Stage</th> ${canSee('financials') ? '<th>Value</th>' : ''} <th>Systems</th> <th>Updated</th> </tr> </thead> <tbody id="proj-tbody"> ${sorted.map(p => projectRow(p)).join('')} </tbody> </table> ${sorted.length === 0 ? '<div class="empty-state"><span class="empty-icon">📋</span>No projects yet. Sync with Jetbuilt or create a new intake.</div>' : ''} </div> `;
}

function projectRow(p) {
const stg = STAGES.find(s => s.key === p.stage) || STAGES[0];
return `<tr onclick="openProject(${p.id})" data-stage="${p.stage}" data-name="${esc(p.name).toLowerCase()}"> <td> <div class="proj-name">${esc(p.name)}</div> <div class="proj-id">#${p.id}</div> </td> <td>${esc(p.client_name || '—')}</td> <td><span class="status-pill status-${stg.color}">${stg.label}</span></td> ${canSee('financials') ?`<td>${fmt(p.total)}</td>`: ''} <td>${p.systems.map(systemTagHTML).join('') || '<span style="color:#6E7681">—</span>'}</td> <td style="font-size:12px;color:#6E7681">${shortDate(p.updated_at)}</td> </tr>`;
}
...
body.innerHTML = `<div class="tabs" id="modal-tabs"> <div class="tab active" onclick="switchModalTab('overview')">Overview</div> <div class="tab" onclick="switchModalTab('design')">Design</div> <div class="tab" onclick="switchModalTab('install')">Install</div> <div class="tab" onclick="switchModalTab('notes')">Notes</div> </div> <div id="modal-tab-content"></div>`;
...
if (tab === ‘overview’) {
tc.innerHTML = `<div class="dashboard-grid"> <div class="dashboard-card"> <div class="dashboard-card-title">Project Info</div> <div style="font-size:13px;color:#C9D1D9;line-height:1.8"> <div><strong style="color:#8B949E">Stage:</strong> ${esc(p.raw_stage || p.stage)}</div> ${canSee('financials') ?`
...
`: ''} <div><strong style="color:#8B949E">Created:</strong> ${fmtDate(p.created_at)}</div> <div><strong style="color:#8B949E">Updated:</strong> ${fmtDate(p.updated_at)}</div> ${p.start_date ?`<div><strong style="color:#8B949E">Install Date:</strong> ${fmtDate(p.start_date)}</div>`: ''} </div> </div> <div class="dashboard-card"> <div class="dashboard-card-title">Client</div> <div style="font-size:13px;color:#C9D1D9;line-height:1.8"> <div><strong style="color:#8B949E">Company:</strong> ${esc(p.client_name || '—')}</div> ${canSee('client_contact') ?`
...
`: ''} ${p.address ?`<div><strong style="color:#8B949E">Address:</strong> ${esc(p.address)}${p.city ? ‘, ’ + esc(p.city) : ‘’}${p.state_abbr ? ’ ’ + esc(p.state_abbr) : ‘’}</div>`: ''} </div> </div> </div> ${p.systems.length ?`
...
tc.innerHTML = `<div class="dashboard-card"> <div class="dashboard-card-title">Project Notes</div> <textarea class="form-textarea" id="project-notes" rows="8" placeholder="Add notes about this project…" oninput="localStorage.setItem('${noteKey}', this.value)">${esc(existing)}</textarea> <div style="margin-top:8px;font-size:11px;color:#6E7681">Notes save automatically</div> </div>`;
}
}
...
cells += `<td class="${isToday ? 'today' : ''}"> <span class="cal-day-num">${dayCount}</span> ${events.map(e => `<div class="cal-event ${e.color}" onclick="openProject(${e.id})" title="${esc(e.name)}">${esc(e.name)}</div>`).join('')} </td>`;
dayCount++;
}
}
cells += ‘</tr>’;
...
c.innerHTML = `<div class="calendar-container"> <div class="calendar-controls"> <div class="calendar-nav"> <button class="cal-btn" onclick="calNav(-1)"> <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M8 2L4 6l4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg> </button> <span class="cal-month">${months[month]} ${year}</span> <button class="cal-btn" onclick="calNav(1)"> <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M4 2l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg> </button> </div> <button class="cal-btn" onclick="calToday()" style="width:auto;padding:0 10px;font-size:11px">Today</button> </div> <table class="calendar-grid"> <thead><tr>${days.map(d =>`<th>${d}</th>`).join('')}</tr></thead> <tbody>${cells}</tbody> </table> </div> `;
}
...
c.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"> <div class="section-title">Shop Work Queue</div> <button class="btn-primary" onclick="addShopWork()">+ Add Task</button> </div> <div class="card"> ${tasks.length === 0 ? '<div class="empty-state"><span class="empty-icon">🔧</span>No shop work tasks. Add tasks for the team when installs get moved or cancelled.</div>' : ''} ${tasks.map((t, i) =>`
...
c.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:8px"> <input type="text" class="form-input" placeholder="Search vendors…" id="vendor-search" oninput="filterVendors()" style="max-width:260px"> <button class="btn-primary" onclick="addVendor()">+ Add Vendor</button> </div> <div class="vendors-grid" id="vendors-grid"> ${vendors.length === 0 ? '<div class="empty-state" style="grid-column:1/-1"><span class="empty-icon">👥</span>No vendors yet. Add your vendor contacts to enable auto-generated quote request emails.</div>' : ''} ${vendors.map((v, i) =>`
...
const progress = `<div style="display:flex;gap:4px;margin-bottom:24px"> ${steps.map(st =>`
...
if (s === 1) {
content = `<div class="card" style="max-width:560px;margin:0 auto"> <h3 style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:16px">Client Information</h3> <div class="form-group"> <label class="form-label">Company / Client Name</label> <input class="form-input" id="int-client" value="${esc(d.client || '')}" placeholder="Search or type client name…"> </div> <div class="form-group"> <label class="form-label">Contact Name</label> <input class="form-input" id="int-contact" value="${esc(d.contact || '')}" placeholder="Primary contact"> </div> <div class="form-row"> <div class="form-group"> <label class="form-label">Email</label> <input class="form-input" id="int-email" type="email" value="${esc(d.email || '')}" placeholder="email@example.com"> </div> <div class="form-group"> <label class="form-label">Phone</label> <input class="form-input" id="int-phone" value="${esc(d.phone || '')}" placeholder="(555) 123-4567"> </div> </div> <div class="form-group"> <label class="form-label">Address</label> <input class="form-input" id="int-address" value="${esc(d.address || '')}" placeholder="Street address, city, state"> </div> </div>`;
} else if (s === 2) {
const venues = [‘Church / Worship’, ‘Event Center’, ‘School’, ‘Theater’, ‘Office / Conference’, ‘Studio / Broadcast’];
content = `<div class="card" style="max-width:560px;margin:0 auto"> <h3 style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:16px">Venue Type</h3> <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"> ${venues.map(v =>`
...
const sel = d.scope || [];
content = `<div class="card" style="max-width:560px;margin:0 auto"> <h3 style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:4px">System Scope</h3> <p style="font-size:12px;color:#6E7681;margin-bottom:16px">Select all systems included in this project</p> <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px"> ${scopes.map(sc => { const active = sel.includes(sc.key); return`<div onclick="toggleScope('${sc.key}')"
...
content = `<div class="card" style="max-width:560px;margin:0 auto"> <h3 style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:16px">Project Details</h3> <div class="form-group"> <label class="form-label">Use Case / Description</label> <textarea class="form-textarea" id="int-usecase" placeholder="Describe how the system will be used…">${esc(d.usecase || '')}</textarea> </div> <div class="form-row"> <div class="form-group"> <label class="form-label">Project Type</label> <select class="form-select" id="int-type"> <option value="">Select…</option> <option value="new" ${d.type === 'new' ? 'selected' : ''}>New Construction</option> <option value="retrofit" ${d.type === 'retrofit' ? 'selected' : ''}>Retrofit / Upgrade</option> </select> </div> <div class="form-group"> <label class="form-label">Timeline</label> <select class="form-select" id="int-timeline"> <option value="">Select…</option> <option value="asap" ${d.timeline === 'asap' ? 'selected' : ''}>ASAP</option> <option value="1-3months" ${d.timeline === '1-3months' ? 'selected' : ''}>1–3 Months</option> <option value="3-6months" ${d.timeline === '3-6months' ? 'selected' : ''}>3–6 Months</option> <option value="6plus" ${d.timeline === '6plus' ? 'selected' : ''}>6+ Months</option> </select> </div> </div> <div class="form-group"> <label class="form-label">Budget Range (optional)</label> <input class="form-input" id="int-budget" value="${esc(d.budget || '')}" placeholder="e.g. $50,000 – $75,000"> </div> <div class="form-group"> <label class="form-label">Owner-Furnished Equipment (OFE)</label> <textarea class="form-textarea" id="int-ofe" rows="3" placeholder="Any existing equipment client wants to keep/reuse?">${esc(d.ofe || '')}</textarea> </div> </div>`;
} else if (s === 5) {
content = `<div class="card" style="max-width:560px;margin:0 auto"> <h3 style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:16px">Quote Structure</h3> <div style="display:grid;gap:8px;margin-bottom:16px"> <div onclick="selectQuoteType('single')" class="card card-sm" style="cursor:pointer;border-color:${d.quoteType === 'single' ? '#1565C0' : '#1C2333'};background:${d.quoteType === 'single' ? '#0D1626' : '#161B22'}"> <div style="font-size:13px;font-weight:500;color:${d.quoteType === 'single' ? '#58A6FF' : '#C9D1D9'}">Single Quote</div> <div style="font-size:11px;color:#6E7681;margin-top:2px">One scope of work, one Jetbuilt project</div> </div> <div onclick="selectQuoteType('gbb')" class="card card-sm" style="cursor:pointer;border-color:${d.quoteType === 'gbb' ? '#1565C0' : '#1C2333'};background:${d.quoteType === 'gbb' ? '#0D1626' : '#161B22'}"> <div style="font-size:13px;font-weight:500;color:${d.quoteType === 'gbb' ? '#58A6FF' : '#C9D1D9'}">Good / Better / Best</div> <div style="font-size:11px;color:#6E7681;margin-top:2px">Three separate Jetbuilt projects — same systems, different tiers</div> </div> </div> ${d.quoteType === 'gbb' ?`
...
`: ''} <div class="form-group" style="margin-top:16px"> <label class="form-label">Payment Approach</label> <select class="form-select" id="int-payment"> <option value="prepay" ${(d.payment || 'prepay') === 'prepay' ? 'selected' : ''}>Prepay Discount (90% upfront = 7% off equipment + 2% off labor)</option> <option value="standard" ${d.payment === 'standard' ? 'selected' : ''}>Standard (100% equipment upfront, labor on completion)</option> <option value="progress" ${d.payment === 'progress' ? 'selected' : ''}>Progress Billing (multi-month project)</option> </select> </div> </div>`;
} else if (s === 6) {
content = `<div class="card" style="max-width:560px;margin:0 auto"> <h3 style="font-size:16px;font-weight:600;color:#E6EDF3;margin-bottom:16px">Notes & Walkthrough</h3> <div class="form-group"> <label class="form-label">Walkthrough Notes</label> <textarea class="form-textarea" id="int-notes" rows="4" placeholder="Observations from the site walkthrough…">${esc(d.notes || '')}</textarea> </div> <div class="form-group"> <label class="form-label">Registration Opportunity</label> <select class="form-select" id="int-registration"> <option value="no" ${(d.registration || 'no') === 'no' ? 'selected' : ''}>No</option> <option value="yes" ${d.registration === 'yes' ? 'selected' : ''}>Yes — register with vendors</option> </select> </div> <div class="form-group"> <label class="form-label">Additional Notes</label> <textarea class="form-textarea" id="int-addnotes" rows="3" placeholder="Anything else…">${esc(d.addnotes || '')}</textarea> </div> </div>`;
}

// Nav buttons
const nav = `<div style="display:flex;justify-content:space-between;max-width:560px;margin:20px auto 0"> ${s > 1 ?`<button class="btn" onclick="intakeNav(-1)">← Back</button>`: '<div></div>'} ${s < 6 ?`<button class="btn-primary" onclick="intakeNav(1)">Next →</button>`:`<button class="btn-primary" onclick="submitIntake()" style="background:#238636">Generate SOW ✓</button>`} </div>`;
...
if (c2) {
c2.innerHTML = `<div style="padding:24px;text-align:center"> <div style="color:#8B949E;font-size:13px;margin-bottom:12px">Welcome to Valiant Integrations</div> <button onclick="syncJetbuilt()" class="btn-primary" style="padding:10px 24px;font-size:14px">Sync Jetbuilt Projects</button> <div style="font-size:11px;color:#6E7681;margin-top:8px">Or use + New Intake to create a project</div> </div>`;
}
}
}
