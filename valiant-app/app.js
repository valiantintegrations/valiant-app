...
<style id="vi-safe-bottom">
  /* iOS home-indicator ("swipe-up bar") clearance + extra bottom breathing room. */
  @media (max-width: 768px) {
    #content { padding-bottom: calc(96px + env(safe-area-inset-bottom)) !important; }
    body.has-context-nav #content { padding-bottom: calc(124px + env(safe-area-inset-bottom)) !important; }
...
<script type="module">
  ...
  const LOCAL_ONLY = new Set(['vi_role', 'vi_user', 'vi_last_read_ch', 'vi_last_read']);
  ...
  // Append-style logs that many people edit at once. Writing the whole array blindly
  // lets a stale copy clobber someone else's just-added item, so for these we union
  // with the current cloud copy (by id) right before writing.
  const MERGE_KEYS = new Set(['vi_messages']);
  window.VI_SYNC_BUILD = 'merge3';
  function _unionById(a, b) {
    const byId = new Map();
    (a || []).forEach(x => { if (x && x.id != null) byId.set(x.id, x); });
    (b || []).forEach(x => { if (x && x.id != null) byId.set(x.id, x); }); // b wins on same id
    return Array.from(byId.values());
  }
  async function _mergeWithCloud(key, localArr) {
    if (!Array.isArray(localArr)) return localArr;
    try {
      const { data } = await sb.from('app_data').select('value').eq('key', key).single();
      const cloudArr = (data && Array.isArray(data.value)) ? data.value : [];
      return _unionById(cloudArr, localArr); // local wins on same id
    } catch (e) { return localArr; }
  }
  ...
  async function mirrorSet(key, raw) {
    ...
    try {
      if (MERGE_KEYS.has(key) && Array.isArray(value)) {
        value = await _mergeWithCloud(key, value);
        pendingWrites.set(key, value);        // track the merged value for confirm/flush
        hydrating = true;
        try { rawSet(key, JSON.stringify(value)); } finally { hydrating = false; }
  ...
  async function hydrateFromCloud() {
    ...
    try {
      if (data && data.length) {
        ...
        clearLocalViKeys();
        data.forEach(row => {
          if (LOCAL_ONLY.has(row.key)) return;  // device-local keys (read state, role) never pulled from cloud
      ...
      sb.channel('app_data_rt')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'app_data' }, (payload) => {
          ...
          if (key && LIVE_KEYS.has(key)) {
            ...
            const applyVal = (raw) => {
              let v = (typeof raw === 'string') ? raw : JSON.stringify(raw);
              if (MERGE_KEYS.has(key)) {
                // Union incoming with what we already have, so a live update can't drop
                // a message this device sent that the sender hadn't seen yet.
                try {
                  const incoming = JSON.parse(v);
                  const localCur = JSON.parse(localStorage.getItem(key) || '[]');
                  if (Array.isArray(incoming) && Array.isArray(localCur)) {
                    const union = _unionById(localCur, incoming);
                    v = JSON.stringify(union);
                    if (union.length > incoming.length) {
                      // We hold messages the cloud copy is missing — push the union back
                      // so the cloud converges instead of losing them.
                      setTimeout(() => { try { mirrorSet(key, v); } catch (e) {} }, 60);
                    }
                  }
                } catch (e) {}
              }
...
<!-- ===== Web Push: paste your VAPID PUBLIC key below (from `npx web-push generate-vapid-keys`) ===== -->
<script>
  window.VI_VAPID_PUBLIC = 'BKBXdIAHhiL1LOh81EirR_dNSjfCuKInHLNQtn2d7RFx0eYLvlTxeoAjLq2xs16C8olOZGblJlpUvffxqn-7F6g';
