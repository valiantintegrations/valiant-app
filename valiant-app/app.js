...
function renderProjectMeetingsSection(p) {
  ...
  function row(m) {
    const dateObj = m.date ? new Date(m.date + 'T00:00:00') : null;
    const dateLabel = dateObj ? dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '—';
    const timeLabel = m.startTime && m.endTime ? _fmt12hRange(m.startTime, m.endTime) : (m.startTime ? _fmt12h(m.startTime) : 'All day');
    const isPending = m.status === 'pending_approval';
    const attendeeCount = (m.attendees || []).length;
    return `
      <div class="proj-meeting-row" onclick="openMeetingDetail(${m.id})">
        <div class="proj-meeting-date">
          <div class="proj-meeting-date-day">${dateLabel}</div>
          <div class="proj-meeting-date-time">${esc(timeLabel)}</div>
        </div>
        <div class="proj-meeting-info">
          <div class="proj-meeting-title">${esc(m.title)}</div>
          <div class="proj-meeting-meta">
            ${esc(_meetingTypeLabel(m.type))}${attendeeCount > 0 ? ` &middot; ${attendeeCount} attendee${attendeeCount === 1 ? '' : 's'}` : ''}
          </div>
        </div>
        ${isPending
          ? `<span class="proj-meeting-status" style="color:#D29922;border-color:#D29922">Pending</span>`
          : `<span class="proj-meeting-status" style="color:#3FB950;border-color:#2C5A33">Confirmed</span>`}
      </div>
    `;
  }
  ...
  return `
    <div class="dashboard-card" style="margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
        <div style="font-size:11px;font-weight:600;color:#6E7681;text-transform:uppercase;letter-spacing:0.08em">Meetings</div>
        <button class="btn btn-sm" style="font-size:11px;padding:6px 12px" onclick="_scheduleNewProjectMeeting(${p.id})">+ Schedule Meeting</button>
      </div>
      ${meetings.length === 0 ? `
        <div style="font-size:12px;color:#6E7681;font-style:italic;padding:4px 0">No meetings scheduled for this project yet.</div>
      ` : `
        ${upcoming.length > 0 ? upcoming.map(row).join('') : ''}
        ${past.length > 0 ? `
          <div style="font-size:10px;color:#6E7681;text-transform:uppercase;letter-spacing:0.05em;margin:12px 0 6px">Past</div>
          <div style="opacity:0.6">${past.map(row).join('')}</div>
        ` : ''}
      `}
    </div>
  `;
...
// ── Settings page ──
// Applies a UI text-size preference as a root class. 'large' bumps a root
// font-size variable that the stylesheet's rem-based and em-based sizes follow,
// plus a scale on key fixed-px areas via the .text-large class.
function applyTextSize(size) {
  const root = document.documentElement;
  root.classList.remove('text-normal', 'text-large');
  root.classList.add(size === 'large' ? 'text-large' : 'text-normal');
}

function setTextSize(size) {
  state.textSize = size;
  localStorage.setItem('vi_text_size', size);
  applyTextSize(size);
  renderCurrentPage();
}
...
function renderSettings(c) {
  ...
  c.innerHTML = `
    ...
    <div style="max-width:640px">
      <div class="dashboard-card" style="margin-bottom:14px">
        <div style="font-size:11px;font-weight:600;color:#6E7681;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:4px">Display</div>
        <div style="font-size:14px;font-weight:600;color:#E6EDF3;margin-bottom:2px">Text size</div>
        <div style="font-size:12px;color:#8B949E;margin-bottom:12px">Make text and controls throughout the app larger for easier reading.</div>
        <div style="display:inline-flex;background:#161B22;border:1px solid #30363D;border-radius:8px;overflow:hidden">
          <button type="button" class="settings-sizebtn${size === 'normal' ? ' is-active' : ''}" onclick="setTextSize('normal')">Normal</button>
...
function openMobilizationDialog(projectId) {
  ...
  overlay.className = 'modal-overlay';
  overlay.innerHTML = `
    <div class="modal-container mob-dialog-container" style="max-width:560px;display:flex;flex-direction:column">
...
// Schedule a brand-new meeting for a project (from the project Meetings list).
// Unlike openProjectMeetingPicker, this NEVER auto-replaces an existing meeting —
// it always creates a fresh one. The user picks the type via the picker.
function _scheduleNewProjectMeeting(projectId) {
  const p = state.projects.find(x => x.id === projectId);
  openMeetingPicker({
    projectId,
    defaultTitle: p ? `Meeting · ${p.name}` : 'Meeting',
    defaultType: 'misc',
    defaultAttendees: [getActiveTeamMemberId()].filter(id => id != null),
    defaultDuration: 30
    // no replacingMeetingId — always creates new
  });
}
...
// ── Meeting detail dialog (Calendar Round 4) ──
// View an existing meeting, with actions: Edit/Reschedule (re-opens the picker
// pre-filled, replacing this meeting on confirm), and Cancel (delete).
function openMeetingDetail(meetingId) {
  document.getElementById('meeting-detail-dialog')?.remove();
  const overlay = document.createElement('div');
  overlay.id = 'meeting-detail-dialog';
  overlay.className = 'modal-overlay';
  overlay.innerHTML = renderMeetingDetail(meetingId);
  overlay.onclick = (e) => { if (e.target === overlay) closeMeetingDetail(); };
  document.body.appendChild(overlay);
  document.body.style.overflow = 'hidden';
}
...
function renderMeetingDetail(meetingId) {
  ...
  if (!m) return `<div class="modal-container"><div style="padding:20px">Meeting not found.</div></div>`;
  const proj = m.projectId ? state.projects.find(p => p.id === m.projectId) : null;
  const timeStr = m.startTime && m.endTime
    ? _fmt12hRange(m.startTime, m.endTime)
    : (m.startTime ? _fmt12h(m.startTime) : 'All day');
  const dateObj = m.date ? new Date(m.date + 'T00:00:00') : null;
  const dateStr = dateObj ? dateObj.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 'No date';
  const attendees = (m.attendees || [])
    .map(id => (state.team.find(t => t.id === id) || {}).name)
    .filter(Boolean);
  const projColor = m.projectId != null ? getProjectColor(m.projectId) : '#A371F7';
  const isPending = m.status === 'pending_approval';

  // Permission to edit/cancel: meeting creator, any attendee, or Master Admin
  const me = getActiveTeamMemberId();
  const canManage = currentUserHasPermission('admin.system')
    || m.createdBy === me
    || (m.attendees || []).includes(me);

  return `
    <div class="modal-container" style="max-width:460px">
      <div style="height:4px;background:${projColor};border-radius:8px 8px 0 0;margin:-1px -1px 0"></div>
      <div style="padding:20px">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:16px">
          <div style="flex:1;min-width:0">
            <div style="font-size:11px;font-weight:600;color:#8B949E;text-transform:uppercase;letter-spacing:0.05em">${esc(_meetingTypeLabel(m.type))}</div>
            <div style="font-size:18px;font-weight:600;color:#E6EDF3;margin-top:2px">${esc(m.title)}</div>
          </div>
          <button onclick="closeMeetingDetail()" style="background:transparent;border:none;color:#8B949E;cursor:pointer;font-size:20px;line-height:1;padding:0">&times;</button>
        </div>

        <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:18px">
          <div style="display:flex;align-items:center;gap:10px">
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="11" rx="1.5" stroke="#8B949E" stroke-width="1.3"/><path d="M2 6h12M5 1.5v2M11 1.5v2" stroke="#8B949E" stroke-width="1.3" stroke-linecap="round"/></svg>
            <span style="font-size:13px;color:#C9D1D9">${esc(dateStr)}</span>
          </div>
          <div style="display:flex;align-items:center;gap:10px">
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6" stroke="#8B949E" stroke-width="1.3"/><path d="M8 4.5V8l2.5 1.5" stroke="#8B949E" stroke-width="1.3" stroke-linecap="round"/></svg>
            <span style="font-size:13px;color:#C9D1D9">${esc(timeStr)}</span>
          </div>
          ${proj ? `
            <div style="display:flex;align-items:center;gap:10px;cursor:pointer" onclick="closeMeetingDetail();openProject(${proj.id})">
              <span style="width:12px;height:12px;border-radius:3px;background:${projColor};flex-shrink:0"></span>
              <span style="font-size:13px;color:#58A6FF">${esc(proj.name)}</span>
            </div>
          ` : ''}
          ${attendees.length > 0 ? `
            <div style="display:flex;align-items:flex-start;gap:10px">
              <svg width="15" height="15" viewBox="0 0 16 16" fill="none" style="margin-top:2px"><circle cx="8" cy="5" r="2.5" stroke="#8B949E" stroke-width="1.3"/><path d="M3 13c0-2.5 2.2-4 5-4s5 1.5 5 4" stroke="#8B949E" stroke-width="1.3" stroke-linecap="round"/></svg>
              <span style="font-size:13px;color:#C9D1D9">${attendees.map(esc).join(', ')}</span>
            </div>
          ` : ''}
          ${isPending ? `
            <div style="display:flex;align-items:center;gap:10px">
              <span style="width:8px;height:8px;border-radius:50%;background:#D29922"></span>
              <span style="font-size:12px;color:#D29922">Pending approval — has a scheduling conflict awaiting attendee response</span>
            </div>
          ` : ''}
          ${m.notes ? `
            <div style="margin-top:4px;padding:10px;background:#0D1117;border-radius:6px;font-size:12px;color:#C9D1D9;border:1px solid #1C2333">
              ${esc(m.notes)}
            </div>
          ` : ''}
        </div>
        ...
        ${canManage ? `
          <div style="display:flex;gap:8px">
            <button class="btn btn-primary" style="flex:1" onclick="closeMeetingDetail();_editMeeting(${m.id})">Edit / Reschedule</button>
            <button class="btn btn-danger" onclick="_cancelMeeting(${m.id})">Cancel Meeting</button>
          </div>
        ` : `
          <div style="font-size:12px;color:#6E7681;font-style:italic;text-align:center">You can view this meeting but only attendees or the organizer can change it.</div>
        `}
      </div>
    </div>
  `;
...
// Re-open the picker pre-filled with this meeting's data. Confirming will
// replace the original (delete + create) via replacingMeetingId.
function _editMeeting(meetingId) {
  const m = getMeetingById(meetingId);
  if (!m) return;
  const durationMin = (m.startTime && m.endTime)
    ? Math.max(15, _timeToMinutes(m.endTime) - _timeToMinutes(m.startTime))
    : 30;
  openMeetingPicker({
    projectId: m.projectId,
    defaultType: m.type,
    ...
    replacingMeetingId: m.id
  });
}

function _cancelMeeting(meetingId) {
  const m = getMeetingById(meetingId);
  if (!m) return;
  if (!confirm(`Cancel "${m.title}"? This permanently removes the meeting.`)) return;
  state.meetings = (state.meetings || []).filter(x => x.id !== meetingId);
  // Clean up any approval records tied to this meeting
  if (state.meetingApprovals && state.meetingApprovals[meetingId]) {
    delete state.meetingApprovals[meetingId];
    save('vi_meeting_approvals', state.meetingApprovals);
  }
  save('vi_meetings', state.meetings);
  closeMeetingDetail();
        ...
        ${allMeetings.length > 0 ? `
          <div class="day-detail-section">
            ...
            ${allMeetings.map(m => {
              ...
              return `
                <div class="day-detail-item" style="border-left-color:${stripeColor};cursor:pointer" onclick="document.getElementById('cal-day-detail-dialog')?.remove();openMeetingDetail(${m.id})">
