    ...
    <header id="topbar">
      ...
      <div class="topbar-right">
        <select id="role-select" onchange="setUserRole(this.value)"
          style="padding:5px 10px;background:#161B22;border:1px solid #30363D;border-radius:6px;color:#8B949E;font-size:11px;font-family:'DM Sans',sans-serif;cursor:pointer">
...
<script type="module">
  ...
  async function mirrorSet(key, raw) {
    ...
    try { await sb.from('app_data').upsert({ key, value }, { onConflict: 'key' }); }
  ...
  function showSetPw(mode) {
    ...
    $('vi-setpw-title').textContent = mode === 'recovery' ? 'Set a new password' : 'Change your password';
  ...
  async function init() {
    try {
      ...
      sb.auth.onAuthStateChange((event) => { if (event === 'PASSWORD_RECOVERY') showSetPw('recovery'); });
      const { data: { session } } = await sb.auth.getSession();
      if (window.location.hash.includes('type=recovery')) { showSetPw('recovery'); }
