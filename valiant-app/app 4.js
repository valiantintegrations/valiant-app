...
function openProject(id) {
  ...
  const c = document.getElementById('content');
  if (c) c.scrollTop = 0;
  // Inject floating back button directly on body so position:fixed anchors to viewport
  injectFloatingBackButton();
}

function injectFloatingBackButton() {
  // Remove any existing one first
  document.getElementById('floating-back-btn-el')?.remove();
  const btn = document.createElement('button');
  btn.id = 'floating-back-btn-el';
